...
export default function UpdateSheduleCourse(props) {
  const {dataDate} = props;

  //giá trị truyền
  //ten buoi hoc
  const [name, setName] = useState(props.item.className);
  //ten giang vien
  const [leturers, setLeturers] = useState(props.item.trainer);
  ...
  const [textdate, setTextdate] = useState(props.item.startedTime);
  const [textdate2, setTextdate2] = useState(props.item.endedTime);
  ...
  //nhan du lieu
  const result = props.databuild;
  useEffect(() => {
    if (result !== null) {
      setDatabuilding(result);
      if (company === null) {
        setCompany(props.item.buildingName);
        if (room === null) {
          setRoom(props.item.roomName);
        }
      }
    }
  }, [result]);
  ...
  const erro = () => {
    ...
    if (
      ...
      //props.addClass(course_id,name,leturers,convertStringToDate(datemode),textdate,textdate2,idcompany,idroom)
      //props.updateClass();
      let input = {
        classId: props.item.classId,
        className: name,
        trainer: leturers,
        date: convertStringToDate(datemode),
        startedTime: textdate,
        endedTime: textdate2,
        buildingId: idcompany,
        roomId: idroom,
      };
      props.updateClass(input);
    }
  };
